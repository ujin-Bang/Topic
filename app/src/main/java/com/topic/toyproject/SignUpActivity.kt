...
//            만약에 이메일 / 닉네임 중복검사를 통과하지 못한 상태라면
//            토스트로 "이메일 중복검사를 통과해야 합니다." 등의 문구만 출력, 가입진행 X

//            hint) 진행할 상황이 아니라면, return처리하면 함수 종료

            if (!(binding.txtEmailCheckResult.text == "사용해도 좋은 이메일입니다.")) {
                Toast.makeText(mContext, "이메일 중복검사를 통과해야합니다.", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (!(binding.txtNicknameCheckResult.text == "사용해도 좋은 닉네임입니다.")) {
