...
import com.topic.toyproject.adapters.ReplyAdapter
...
//            투표를 하지 않은 상태라면, 댓글 작성도 불가.
            ...
            if (mTopicData.mySelectedSide == null){
                Toast.makeText(mContext, "의견을 개진할 진영을 선택하셔야 댓글작성 가능합니다.", Toast.LENGTH_SHORT).show()

//                클릭 이벤트 자체를 강제 종료(Intent 실행을 막자)
                return@setOnClickListener //return : 함수의 결과를 지정 => 그 뒤의 코드는 실행X=> 함수 강제종료.
            }
...
//        btnVote1클릭 => 첫 진영의 id값을 찾아서 거기에 투표.
//        서버에 전달 => API활용
        binding.btnVoteCount.setOnClickListener {

//            서버의 투표API 호출
            ServerUtil.postRequestVote(mContext, mTopicData.sideList[0].id, object : ServerUtil.JsonResponseHandler{
                override fun onResponse(jsonObj: JSONObject) {

//                    토스트로 서버가 알려준 현재상황( 신규투표 or 재투표 or 취소 등)
...
//                첫 진영에 투표한 경우.
                binding.btnVoteCount.text = "투표 취소"
                binding.btnVoteCount2.text ="다시 투표"
            }
            else {
//                두번째 진영에 투표.
                binding.btnVoteCount.text = "다시 투표"
                binding.btnVoteCount2.text = "투표 취소"
            }
        }
        else {
//            아무데도 투표하지 않은 경우
            binding.btnVoteCount.text = "투표하기"
            binding.btnVoteCount2.text = "투표하기"
        }
...
//                토론정보 JSONObject(topicObj) = TopicData() 형태로 변환(여러 화면에서 진행. 함수로 만들어두자)
...
//                => 기존에 다른 댓글이 들어있다면. 그 뒤에 이어서 추가된다.
//                => 기존 댓글 목록을 전부 삭제하고 나서 추가하자.

                mReplyList.clear()
//                topicObj 내부에는 replies라는 댓글 목록JSONArray도 들어있다.
//                mReplyList에 넣어주자.
