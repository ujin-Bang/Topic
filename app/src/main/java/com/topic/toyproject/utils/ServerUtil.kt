...
//        로그인 기능 호출 함수.
//        handler: 이 함수를 쓰는 화면에서 JSON분석을 어떻게 / UI에서 활용할지 방안(인터페이스)
//        -처리 방안을 임시로 비워두려면, null대입 허용
...
//          종합한 Request도 실제 호출을 해 줘야 API호출이 실행됨(startActivity 같은 동작 필요함)
...
//                    ex. 인터넷 끊김, 서버접속 불가 등등 물리적 연결 실패
//                    ex. 비번이 틀려서 로그인 실패: 서버 연결 성공, 응답도 돌아왔는데 > 그 내용만 실패(물리적 실패가 아니다)
...
//                    응답 : response 변수 > 응답의 본문만(body) 보자.
...
//        토큰값 자체는 파라미터로 받아올 필요 없다. => ContextUtil을 불러서 사용하자.
...
//
        ...
        fun postRequestLikeOrHate( context: Context, reply_id: Int, islike: Boolean, handler: JsonResponseHandler?){
